...
class DATA():
    ...
    def get_branch_data(self, st, en):
        ...
        for i in range(st, en):
            for k in range(2):
                if k == 0:
                    FILE_DIR = cf.DATA_PATH + '噪声训练集/' + chr(ord('E')) + '/'
                else:
                    tt = ord('E') + (i % 4 + 1)
                    ...
                    FILE_DIR = cf.DATA_PATH + '噪声训练集/' + chr(tt) + '/'
...
class MODEL():
    ...
    def assemble_graph(self, w_alpha = 0.01, b_alpha=0.01):
        ...
        w_out = tf.Variable(w_alpha*tf.random_normal([4 * 4 * 64, 1]), dtype=tf.float32)
    ...
    def train(self, size = 11000, w_alpha = 0.01, b_alpha=0.01):
        ...
        with tf.Session() as sess:
            sess.run(tf.global_variables_initializer())
            #saver.restore(sess, cf.MODEL_PATH + 'CNN.model-9891')
            ...
            while True: 
                ...
                _, L = sess.run([op,loss], feed_dict={self.X:X[step % BRANCH_NUM], self.Y:Y[step % BRANCH_NUM], self.keep_prob:0.4})
