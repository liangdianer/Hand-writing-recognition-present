...
class DATA():
    ...
    def get_train_data(self, size=11000):
        ...
        BRANCH_NUM = 3#int(size/(cf.BRANCH_SIZE * 5))
...
class MODEL():
    ...
    def train(self, size = 11000, w_alpha = 0.01, b_alpha=0.01):
        ...
        loss = tf.reduce_mean(loss)# + (50 * self.L2)
        ...
        with tf.Session() as sess:
            #sess.run(tf.global_variables_initializer())
            saver.restore(sess, cf.MODEL_PATH + 'CNN.model-9812')
            ...
            while True: 
                '''
                _, L = sess.run([op,loss], feed_dict={self.X:X[step % BRANCH_NUM], self.Y:Y[step % BRANCH_NUM], self.keep_prob:0.6})
